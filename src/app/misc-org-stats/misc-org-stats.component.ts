...
export class OrgStatsComponent implements OnInit {
  ...
  getOrgData() {
    this.loading = true;
    this.idnService.getHostingData().subscribe(result => {
    ...
    this.idnService.getAccountCount().subscribe(result => {
      this.orgData.accountCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getIdentityProfileCount().subscribe(result => {
      this.orgData.identityProfileCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getIdentityCount().subscribe(result => {
      this.orgData.identityCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getSourceCount().subscribe(result => {
      this.orgData.sourceCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getTotalAccessProfilesCount().subscribe(result => {
      this.orgData.accessProfileCount = result;
    });
    ...
    this.idnService.getTotalCampaignCount().subscribe(result => {
      this.orgData.totalCampaignCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getActiveCampaignCount().subscribe(result => {
      this.orgData.activeCampaignCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getCompletedCampaignCount().subscribe(result => {
      this.orgData.completedCampaignCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getPasswordChangeCount().subscribe(result => {
      this.orgData.passwordChangeCount = result.headers.get('X-Total-Count');
    });

    this.idnService.getProvisioningActivityCount().subscribe(result => {
      this.orgData.provisioningActivityCount =
